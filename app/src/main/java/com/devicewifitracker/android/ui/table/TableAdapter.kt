...
class TableAdapter(private val tableList: List<String>) : RecyclerView.Adapter<TableAdapter.ViewHolder>() {

    class ViewHolder(tableItemBinding: IpTableItemBinding) : RecyclerView.ViewHolder(tableItemBinding.root) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val tableItemBinding = IpTableItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        val holder = ViewHolder(tableItemBinding)
        holder.itemView.setOnClickListener {
            val position = holder.bindingAdapterPosition
            val table = tableList[position]
//            DataActivity.actionOpenTable(parent.context, table.name)
            SuspiciousDetailActivity.actionOpenAct(parent.context,table)
        }
        return holder
    }
...
//        holder.setExtraMarginForFirstAndLastItem(position == 0, position == tableList.size - 1)
        ...
        if (NetworkUtils.getIpAddressByWifi() .equals(ip) || NetworkUtils.getGatewayByWifi().equals(ip)) {
...
//        LogUtils.e("ip=="+"${ip}-------${NetworkUtils.getGatewayByWifi() }---${NetworkUtils.getIpAddressByWifi()}")
            when (ip) {
                NetworkUtils.getGatewayByWifi() ->{
                    ...
                    holder.tabEquipmentText.text = "Router"
                }
                NetworkUtils.getIpAddressByWifi() ->{
                    Glide.with(App.context).load(R.mipmap.icon_phone).into(holder.image)
                    holder.tabEquipmentText.text = "My Phone"
                }
            }
        } else {
            holder.tabEquipmentText.text = "Unknowed"
            holder.tabEquipmentText.setTextColor(
                ContextCompat.getColor(
                    App.context,
                    R.color.red_ff0039
